...
export default function EditPedidoPage() {
  ...
  const { data: pedido, loading: loadingPedido, error: errorPedido } = useApi(() => api.pedido(orderId));
  const { data: orderProjects, loading: loadingProjects } = useApi(() => api.pedidoProjects(orderId));
  const { data: allProjects, loading: loadingAll } = useApi(() => api.projects());
  ...
  useEffect(() => {
    if (pedido) {
      ...
      setStatus(pedido.status || "ACTIVE");
  ...
  useEffect(() => {
    if (orderProjects) {
      setSelectedIds(new Set(orderProjects.map((p: any) => p.id)));
    }
  }, [orderProjects]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await api.updatePedido(orderId, {
        name: name.trim(),
  ...
  if (loadingPedido || loadingProjects || loadingAll) return <Loader />;
  if (errorPedido) return <ErrorMessage message={errorPedido} />;

  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        <Link href={`/pedidos/${orderId}`} className="text-gray-400 hover:text-gray-600">
          <ArrowLeft size={20} />
        </Link>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div>
              ...
              <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className={INPUT_CLS} />
            </div>
            <div>
